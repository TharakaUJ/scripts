...
for line in lines:
    thibus_char_start_at = line.find(thibus_start, 0)
    thibus_char_end_at = line.find(thibus_end, thibus_char_start_at+1)

    thibus_char = line[thibus_char_start_at+1:thibus_char_end_at]

    sin_char_start_at = line.find(sin_start, thibus_char_end_at+1)
    sin_char_end_at = line.find(sin_end, sin_char_start_at+1)

    sin_char = line[sin_char_start_at+1:sin_char_end_at]

    char_dict[re.sub(r'.', unicode_replacement, sin_char)[2:]] = thibus_char

# print(char_dict)

for i in range(3456, 3583, 1):
    hex_val = "0" + hex(i)[2:].upper()
    if hex_val in char_dict:
        hex_dict[char_dict.get(hex_val)] = hex_val
